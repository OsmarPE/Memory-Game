...
export default function Card({verifyIndex,index,setValues,icon,player}:Props) {
    return (
        <button disabled={player === 0 ? true : verifyIndex(index)} onClick={() => setValues(index)} key={index} className='relative border-2 border-black w-full aspect-square text-3xl flex items-center justify-center text-white shadow-main200 bg-purpleMain200 rounded-full'>
